...
def validate_classifier(feature_extractor, classifier, data_loader):
    total = 0
    correct = 0
    feature_extractor.eval()
    classifier.eval()
    ...
    for iteration, batch in enumerate(data_loader):
        ...
        correct_sum = get_correct_sum(out, y)

        correct += correct_sum.item()
        total += y.shape[0]

    return np.round(100 * correct / total, 3)
...
class OfflineArchitectureValidator:
    ...
    def test_architecture(self, args, feature_extractor, classifier, device, numb_epoch=100, lr=1e-3,
                          scheduler_rate=0.99):
